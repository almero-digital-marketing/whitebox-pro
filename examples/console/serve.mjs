...
// Dev convenience: hand the console the analytics bearer token from the
// server's .env so the tester doesn't have to paste it. Local-only — the token
// never leaves localhost. (The field stays editable in the UI.)
async function readAnalyticsToken() {
  try {
    const env = await readFile(path.join(SERVER_DIR, '.env'), 'utf8')
    const m = env.match(/^\s*WB_ANALYTICS_TOKEN\s*=\s*"?([^"\n]*)"?/m)
    return m ? m[1] : ''
  } catch { return '' }
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url === '/console/config') {
      ...
      return res.end(JSON.stringify({ analyticsToken: await readAnalyticsToken() }))
    }
...
server.listen(PORT, async () => {
  console.log(`WhiteBox console [auto-start server] → http://localhost:${PORT}`)
