...
import { mailgun } from 'whitebox-pro-mail-mailgun'   // or: import { postmark } from 'whitebox-pro-mail-postmark'
...
The provider-specific code (transport, webhook signature, payload parsing) lives **outside** this package — in the external `whitebox-pro-mail-mailgun` / `whitebox-pro-mail-postmark` repos — each implementing: `send(msg)→{messageId}`, `verifySignature(req, kind)`, `parseInbound(req)`, `parseTracking(req)`, and optionally `ownsAddress` / `classifyError`.
...
The plugin tests inject a fake provider, so they cover the plumbing regardless of provider. Each provider package (`whitebox-pro-mail-mailgun`, `whitebox-pro-mail-postmark`) ships its own tests for send/verify/parse. No test here for: `mailer.js`, `attachments.js`, `templates.js`, full worker integration (mocked everywhere).
