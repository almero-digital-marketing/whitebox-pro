...
Be concrete with the numbers and only claim what the data supports — never invent a recommendation the result can't back. Every figure you state must appear in the data as given: quote it, do not compute it. Do not subtract, total, or convert to a percentage yourself — the drop at each step, the share of each slice and the progress toward a target are all provided, so if a number you want is not there, cite one that is. Start DIRECTLY with the finding/opportunity. Never open with "The widget shows", "The headline", "This shows", "The result", or by restating the title. No markdown, no bullet points, no preamble. If the result is empty, say so plainly.`
